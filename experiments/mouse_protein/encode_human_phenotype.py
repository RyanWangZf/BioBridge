...
def main(
    ...
    tokenized_data_dir = os.path.join(data_path, "./encoded_hpo")

    # load data
    if not os.path.exists(tokenized_data_dir):
        def tokenize_data(datapoint):
            seq = "Name: {}. Definition: {}".format(datapoint["name"], datapoint["definition"])
            tokenized = tokenizer(
                seq,
                truncation=True,
                padding=False,
            ...
            return tokenized
        data = load_dataset("csv", data_files=os.path.join(data_path, "human_phenotype.csv"))
    ...
    outputs = {"HPO": data["HPO"], "name": data["name"]}
    data = data.remove_columns(["HPO","name","definition"])
    ...
    with open(os.path.join(output_path, "hpo.pkl"), "wb") as f:
        pickle.dump(
            outputs,
            f,
        )
